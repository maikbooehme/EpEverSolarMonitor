...
/*
    ...
    For taking the data to grafana, have a look here: 
     ...
     Maik: I added the integration via mqtt to iobroker with user and password . This was tested and is successfully working with a Epever tracer 3210AN via Wemos D1 mini and MAX485. I took the 5V from the USB port and powered the Wemos D1 mini and the MAX485 with 5V . Important, not to use the Epever RJ45 PIN 1 !!! 
	 I only used PIN for A / B / GND .
	 
	 Credits to all people involved above.
...
const char* ssid = "your ssid"; // SSID des vorhandenen WLANs
const char* password = "your password"; // Passwort für das vorhandene WLAN
const char* mqtt_server = "your mqtt server";
const char* mqtt_port = "your mqtt server port";
const char* mqtt_user1 = "your mqtt username";                 //mqtt client user
const char* mqtt_pwd1 = "your mqtt password";             //mqtt client password 
const char* mqtt_clientid1 ="EpSolarMonitor";   //mqtt client id
...
void setup(){
    ...
    // mqtt init
    mqtt_client.setServer(mqtt_server, 1883);  //change your mqtt server port here
    mqtt_client.setCallback(mqtt_callback);
        if (mqtt_client.connect(mqtt_clientid1, mqtt_user1 , mqtt_pwd1)) {
            mqtt_client.publish("outTopic","hello mqtt init");
...
void mqtt_reconnect() {
  ...
  while (!mqtt_client.connected()) {
    ...
    // Create a client ID
    String clientId = (mqtt_clientid1);  //mqtt_client id 
    
    // Attempt to connect
    if (mqtt_client.connect(mqtt_clientid1 ,"nuc" ,"Drag0n35")) {
